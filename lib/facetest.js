...
function sendFriendRequest(fbConfig, reqFrom, reqTo, callback) {
  fbgraph
    .setAppSecret(fbConfig.appSecret)
...
function sendAndAcceptFriendRequest(fbConfig, reqFrom, reqTo, callback) {
  sendFriendRequest(fbConfig, reqFrom, reqTo, function (err, res) {
    if (err) {
      callback(err, res, reqFrom, reqTo);
    } else {
      sendFriendRequest(fbConfig, reqTo, reqFrom, function (err, res) {
        callback(err, res, reqFrom, reqTo);
      });
    }
  });
...
FaceTest.prototype.removeFacebookUser = function (name, callback) {
  var self  = this;
  removeTestUser(this.fbConfig, this.facebookUsers[name], function (err, res) {
...
FaceTest.prototype.friendUser = function(reqFrom, reqTo, callback) {
  ...
  sendAndAcceptFriendRequest(this.fbConfig, users[reqFrom], users[reqTo], function (err, res) {
...
FaceTest.prototype.createUser = function (name, callback) {
  ...
  sendNewUserRequest(this.fbConfig, params, function (err, res) {
    if (!err) {
      var user                 = {};
      user[name]               = res;
